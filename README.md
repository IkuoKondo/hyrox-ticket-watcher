...
- 実行場所：**GitHub Actions（無料）** で 5 分おきに自動チェック
- 通知先：**Slack**
...
- **チェック間隔は最短5分**です。さらに GitHub の混雑時は数分ずれたり、まれに間引かれます（ベストエフォート）。1〜2分を争う争奪戦では取りこぼす可能性があります。
...
`.github/workflows/watch.yml` の `cron: "*/5 * * * *"` を変えると間隔を調整できます（5 分未満は不可）。
